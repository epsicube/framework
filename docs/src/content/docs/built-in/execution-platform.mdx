...
1. ### Install the Package
    ...
    ```bash
    composer require unigale/module-execution-platform
    ```
...
Modules can inject custom Activities and Workflows through **[Integrations](/writing-module/integrations/)**.
When the Execution Platform is active, you can extend its registries:

```php
\UniGaleModules\ExecutionPlatform\Facades\Activities::register(...);
\UniGaleModules\ExecutionPlatform\Facades\Workflows::register(...);
```
