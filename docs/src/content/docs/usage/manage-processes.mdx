...
The **`unigale:work`** command starts all module-defined long-running tasks and supervises them continuously.

```bash
php artisan unigale:work
```
...
<Aside>
By default, `unigale:work` manages both the [Laravel Scheduler](https://laravel.com/docs/scheduling#running-the-scheduler) and the [Queue Worker](https://laravel.com/docs/queues#running-the-queue-worker).
If you already run scheduling via `crontab`, or if your queue workers are externally supervised, you can safely replace those setups with `unigale:work`.
</Aside>
...
```bash
php artisan unigale:reload
```

This broadcasts a restart signal that forces `unigale:work` to gracefully stop and restart each managed task:
...
Supervisor configuration files are typically stored in the `/etc/supervisor/conf.d` directory. Within this directory, you may create any number of configuration files that instruct supervisor how your processes should be monitored. For example, let's create a `unigale-worker.conf` file that starts and monitors `unigale:work` processes:

```ini
[program:unigale-worker]
process_name=%(program_name)s_%(process_num)02d
command=php /path/to/your/application/artisan unigale:work
autostart=true
autorestart=true
stopasgroup=true
killasgroup=true
...
sudo supervisorctl start "unigale-worker:*"
```
